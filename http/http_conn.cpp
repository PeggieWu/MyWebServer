//
// Created by ubuntu on 2021/12/4.
//
#include "http_conn.h"
#include "../log/log.h"
...
void http_conn::inint_mysql_result(connection_pool *connPool) {
    //先从连接池中取一个连接
    MYSQL *mysql = nullptr;
...
void http_conn::init(int sockfd, const sockaddr_in &addr, char * root, int TRIGMode, int close_log, string user, string passwd,
    ...
    //当浏览器出现连接重置时，可能是网站跟目录出错或http响应格式出错或者访问打文件中内容完全为空
    doc_root = root;
    m_TRIGMode = TRIGMode;
...
http_conn::HTTP_CODE http_conn::parse_headers(char *text) {
    ...
    {
        LOG_INFO("opps!unkown header:%s",text);
    }
    return NO_REQUEST;
}
//判断http请求是否被完整读入
...
http_conn::HTTP_CODE http_conn::do_request() {
    ...
    //获得文件的属性，放入m_file_stat
    //调用stat函数可以获取到由参数（文件路径）指定的文
...
void http_conn::unmap() {
    ...
    if(m_file_address)
    {
        ...
        // 最后一个页不被使用的空间将会清零。munmap执行相反的操作，删除特定地址区域的对象映射。
...
bool http_conn::add_response(const char *format, ...) {
    ...
    //则将字符串的前size个字符复制到str，同时将原串的长度返回（不包含终止符）；执行失败，返回负值，并置errno.
